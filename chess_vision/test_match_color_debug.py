"""
模板匹配测试工具 - 彩色匹配版本（带调试功能）
保存目标位置的图像，方便检查分割是否正确
"""
...
CORRECT_POSITION = [1, 5]          # 标准答案位置 [行, 列]（从1开始）
SAVE_TARGET_IMAGE = True           # 是否保存目标位置的图像
# =================================
...
def match_template_color(cell, template):
    """彩色模板匹配（保留RGB三通道）"""
    # 调整模板大小到格子大小
    ...
    # 直接用彩色图像匹配（BGR三通道）
    result = cv2.matchTemplate(cell, template_resized, cv2.TM_CCOEFF_NORMED)
    _, max_val, _, _ = cv2.minMaxLoc(result)
    
    return max_val


def test_template_matching(template_path, threshold, use_region_select, correct_pos, save_target):
    """测试模板匹配"""
    
    print("=" * 60)
    print("模板匹配测试工具 - 彩色匹配版本（带调试）")
    print("=" * 60)
    print()
    ...
    print(f"🎨 匹配模式: 彩色匹配（BGR三通道）")
    print(f"🎯 匹配阈值: {threshold}")
    print(f"✅ 标准答案: 位置 {correct_pos[0]}-{correct_pos[1]}")
    print()
    ...
    # 保存检测到的棋盘图像
    if save_target:
        board_filename = "detected_board_color.png"
        cv2.imwrite(board_filename, board_image)
        print(f"💾 已保存检测到的棋盘: {board_filename}")
        print(f"   尺寸: {board_image.shape[1]} x {board_image.shape[0]}")
    ...
    if save_target:
        ...
        for cell, (row, col) in cells:
            if row == target_row and col == target_col:
                filename = f"{row}-{col}_target.png"
                cv2.imwrite(filename, cell)
                print(f"💾 已保存目标位置图像: {filename}")
                print(f"   尺寸: {cell.shape[1]} x {cell.shape[0]}")
                break
        print()
    
    print(f"🎯 步骤3: 彩色模板匹配（阈值 > {threshold}）...")
    print()
    ...
    for cell, (row, col) in cells:
        score = match_template_color(cell, template)
        
        # 记录目标位置的相似度
        if row == correct_pos[0] and col == correct_pos[1]:
            target_score = score
    ...
    if target_score is not None:
        print(f"🎯 标准答案位置 {correct_pos[0]}-{correct_pos[1]} 的相似度: {target_score:.2%}")
        if target_score > threshold:
            print("   ✅ 已匹配（超过阈值）")
        else:
            print(f"   ❌ 未匹配（低于阈值 {threshold:.0%}）")
        print()
    ...
    if matches:
        ...
        for row, col, score in matches:
            marker = " ← 标准答案" if (row == correct_pos[0] and col == correct_pos[1]) else ""
            print(f"  位置 {row}-{col}: 相似度 {score:.2%}{marker}")
    else:
        print("未找到匹配位置")
        print()
        print("建议：")
...
if __name__ == "__main__":
    test_template_matching(TEMPLATE_PATH, MATCH_THRESHOLD, USE_REGION_SELECT, 
                          CORRECT_POSITION, SAVE_TARGET_IMAGE)
