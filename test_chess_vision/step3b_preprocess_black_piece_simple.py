...
# ============ 用户配置 ============
# 输入图像路径（格子图像）
INPUT_IMAGE = "./step1_output/cell_01-04.png"  # 修改这里指定要处理的格子图像

# 输出目录
OUTPUT_DIR = "step3b_output"
...
def extract_black_piece_mask(cell_image):
    ...
    # 2. 创建黑色掩码
    mask_black = cv2.inRange(hsv_image, 
                            PieceProcessConfig.BLACK_HSV_LOWER, 
                            PieceProcessConfig.BLACK_HSV_UPPER)
    print(f"   黑色范围: H={PieceProcessConfig.BLACK_HSV_LOWER[0]}-{PieceProcessConfig.BLACK_HSV_UPPER[0]}°, "
          f"S={PieceProcessConfig.BLACK_HSV_LOWER[1]}-{PieceProcessConfig.BLACK_HSV_UPPER[1]}, "
          f"V={PieceProcessConfig.BLACK_HSV_LOWER[2]}-{PieceProcessConfig.BLACK_HSV_UPPER[2]}")
    ...
    print(f"   黑色像素: {black_pixels}/{total_pixels} ({black_ratio*100:.2f}%)")
...
def process_black_piece(cell_image, save_intermediate=True, output_dir="output"):
    """
    处理黑色棋子图像（简单方法）
    ...
    # 步骤1：提取黑色掩码
    mask_black = extract_black_piece_mask(cell_image)
    
    if save_intermediate:
        cv2.imwrite(os.path.join(output_dir, "01_black_mask.png"), mask_black)
        print(f"   💾 已保存: 01_black_mask.png")
...
def main():
    ...
    # 处理黑色棋子
    processed_image, info = process_black_piece(cell_image, SAVE_INTERMEDIATE, OUTPUT_DIR)
    ...
    if SAVE_INTERMEDIATE:
        ...
        print(f"   {OUTPUT_DIR}/01_black_mask.png       - 黑色掩码")
    ...
    print("\n💡 提示:")
    print("   - 如果效果不好，可能需要使用复杂方法（边缘检测+轮廓缩放）")
    print("   - 检查 01_black_mask.png 看黑色提取是否准确")
    print("   - 检查 04_final_processed.png 看最终结果是否可用")
